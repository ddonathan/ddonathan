...
app.post("/register", (req, res) => {
  ...
  db.run(
    `INSERT INTO users (email, password, workoutDays) VALUES (?, ?, ?)`,
    [email, password, workoutDays],
    function (err) {
      if (err) {
...
app.post("/edit-profile", checkAuth, (req, res) => {
  const { email, password, name, age, weight, workoutDays, timezone } =
    req.body;
  db.run(
    `UPDATE users SET email = ?, password = ?, name = ?, age = ?, weight = ?, workoutDays = ?, timezone = ? WHERE id = ?`,
    [
      email,
      password,
      name,
...
app.get("/profile", checkAuth, (req, res) => {
  db.get(
    `SELECT email, name, age, weight, workoutDays, timezone FROM users WHERE id = ?`,
    [req.session.userId],
    (err, user) => {
      if (err) {
...
app.post("/addLift", checkAuth, (req, res) => {
  ...
  const utcDate = new Date(date).toISOString(); // Convert to UTC in ISO 8601 format
  db.run(
    `INSERT INTO progress (userId, exercise, weight, reps, date) VALUES (?, ?, ?, ?, ?)`,
    [req.session.userId, exercise, weight, reps, utcDate],
    function (err) {
      if (err) {
...
app.get("/getLastPerformance", checkAuth, (req, res) => {
  ...
  db.get(
    `SELECT workoutDays, timezone FROM users WHERE id = ?`,
    [req.session.userId],
    (err, user) => {
      if (err) {
    ...
    today.setUTCHours(0, 0, 0, 0); // Set the time to midnight UTC to avoid issues
      ...
      for (let i = 0; i < 14; i++) {
        ...
        if (workoutDays.includes(dayOfWeek)) {
          dates.push(date.toISOString().split("T")[0]); // Use ISO format consistently
        }
      }

      console.log("Generated Dates:", dates);

      // Define the workout schedule (6-day cycle)
      const schedule = [
      ...
      lifts.forEach((lift, index) => {
        db.get(
          `SELECT * FROM progress WHERE userId = ? AND exercise = ? ORDER BY date DESC LIMIT 1`,
          [req.session.userId, lift],
          (err, row) => {
            if (err) {
            ...
            const nextWeight =
              lastPerformance.weight !== null && lastPerformance.reps >= 5
                ? lastPerformance.weight +
                  (lift === "Bench Press" || lift === "Overhead Press"
                    ? 2.5
                    : 5)
                ...
                dates.forEach((date, i) => {
                    console.log('Checking date:', date); // Log date format here
                    const dayIndex = i % 6; // Cycle through the 6-day schedule
                    if (schedule[dayIndex].includes(lift)) {
                        const existingResult = results.find(result => result.date === date);
                        if (existingResult) {
                            console.log('Date exists, updating:', date);
                            existingResult[lift.toLowerCase().replace(' ', '')] = nextWeight;
                        } else {
